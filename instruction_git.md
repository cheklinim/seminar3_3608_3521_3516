...
Эта команда добавляет удалённый репозиторий, находящийся по адресу URL.
...
### Connction ~~people~~ repositories

Перед началом обмена информацией локального и удалённого репозитория их необходимо соединить. Для этого воспользуемся командой, наблюдаймой нами в предыдущем разделе:

    git remote add <repo_name> <link>

< repo_name > - имя удалённого репозитория

< link > - ссылка на это репозиторий
...
Помните команду *git push -u origin main* из настройки? Так вот, она создаёт поток для загрузки (upstream) для определённой ветки в репозитории. Т.е.:

    git branch -M main
    git push -u origin main

Сначала мы насильно создали (переименовали) ветку, а потом сделали её каналом для передачи. Говоря иначе *git push* у вас будет работать только из той ветки, где он настроен.
...
Есть один момент: перед началом работы с чужим репозиторием для внесения изменений необходимо создать отдельную ветку и там всё менять.
