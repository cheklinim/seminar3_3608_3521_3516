...
При создании репозитория на сайте на главной странице нажимаем зелённую кнопку New. Переходим на страницу создания репозитория, пишем имя, оставляем по умолчанию и жмём кнопку Create repository. Удалённый репозиторий создан.
 ...
 Теперь копируемый репозиторий можно перенести на локальный, для этого на сайте находим зелёную кнопку Code, нажимаем её, выскакивает подменю с адресом копируемого репозитория. Копируем. Переходим в локальный репозиорий, создаём пустую папку. Запускаем терминал. В терминале набираем команду:

    git clone ссылка на сайт

Нажимаем Enter. Git копирует на локальный репозиторий, полную версию удалённого. После тогокак скопировали файлы, нужно их интегрировать в Git. Для этого мы снова запускаем терминал командой Open in integrated Terminal.

Создание копии публичного репозитория на локальномрепозитории:

    git clone

Работая с публичным репозиторием, нужно придерживаться правила хорошего тона. Гласит примерно так - скачал чужой репозиторий - сразу делай свою ветку.
Закончив работать над пректом, отправляем обратно на удалённый репозиторий командой:
    
    git push --set apstream oring remoted

Переходим на сайт, в личный кабинет. Смотрим на запушенный репозиторий. Проверяем. Видно, сто появился  какой-то новый пуш.

Что бы предложить измения нажимаем кнопку Compare & pull request. 
Переходим на страницу. Пишем - предлогаю изменеия. Выбераю свою ветку и пишем сообщение первоначальному хозяину репозитория с предлагаемыми поправками. Жмём зелёную кнопу Create pull reqiust.