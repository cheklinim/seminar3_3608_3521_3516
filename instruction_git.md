...
**Pull request** - команда, которая отправляет запрос на изменения в чужой репозиторий. Предлагать изменения нужно в отдельной ветке.

Чтобы выполнить **pull request** нужно:

    1. Делаем fork (ответвление) чужого репозитория
